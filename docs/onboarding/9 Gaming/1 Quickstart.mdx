...
### Installing web3sdks SDK

Let's import the web3sdks SDK so we can add some web3 functionality!

First, head over to the [`web3sdks/unity-sdk` releases page](https://github.com/web3sdks/unity-sdk/releases) on GitHub.
...
From the `Scenes` folder, double-click the `Web3sdksDemoScene.unity` file to open it. If you manoeuvre around the scene, you'll be able to see the `Canvas` that comes as part of the demo:

![image.png](./assets/7.png)

To view the code powering this demo and see what you can do with the SDK, open the `Web3sdksSDKDemos.cs` file by double-clicking it:
...
// Import the Web3sdks SDK namespace
using Web3sdks;

public class Web3sdksSDKDemos : MonoBehaviour
{
    // Create a Web3sdks SDK instance for us to use throughout this class
    private Web3sdksSDK sdk;

    void Start()
    {
        // When the app starts, set up the Web3sdks SDK
...
Next, click `Player Settings...` (bottom left corner) and set the `WebGL Template` under `Resolution and Presentation` to `Web3sdks`:
...
We can then use [Storage](https://docs.web3sdks.com/storage) to upload and deploy our application to IPFS, by running the following command:

```bash
npx web3sdks@latest upload .
```
