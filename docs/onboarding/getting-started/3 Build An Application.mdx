...
Inside the [`index.js`](https://github.com/web3sdks-template/cra-javascript-starter/blob/main/src/index.js#L15-L17) file,
you'll find the [`Web3sdksProvider`](/sdk/set-up-the-sdk/frontend#manual-installation) wrapping the entire application.
...
which declares [which blockchain & network](https://blog.web3sdks.com/guides/which-network-should-you-use/) our smart contracts live on.
  ...
  return (
    <>
      {isReadingNfts ? (
        ...
        <div>
          {nfts?.map((nft) => (
            <Web3sdksNftMedia metadata={nft.metadata} key={nft.metadata.id} />
          ))}
        </div>
      )}
    </>
...
import { Web3Button } from "@web3sdks/react";
...
To [host your application on IPFS](https://blog.web3sdks.com/guides/how-to-host-your-web-app-on-ipfs/)
...
<TwitterCard
  ...
  text={`Probably nothing 👀 @web3sdks
