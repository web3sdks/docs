...
async function makeExamplePages(repos) {
  ...
  await Promise.all(
    repos.map(async (repo) => {
      const url = `https://raw.githubusercontent.com/web3sdks-template/${repo.name}/main/README.md`;
...
---`;
  ...
  const createSnippet =
    "To create a new project using this template, use the [web3sdks CLI](https://github.com/web3sdks/web3sdks-cli):" +
    "\n" +
    "```jsx" +
    "\n" +
    "npx web3sdks@latest create --template " +
    page.name +
    "\n" +
    "```";
